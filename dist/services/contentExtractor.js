...
export class ContentExtractorService {
     ...
     */
    async extractContent(html, url, platform) {
    ...
    setupTurndownRules() {
        ...
        this.turndownService.addRule('alerts', {
            filter: (node) => {
                ...
                return ['alert', 'callout', 'admonition', 'note', 'warning', 'tip'].some(cls => Array.from(classList).some((c) => c.includes && c.includes(cls)));
            },
            replacement: (content, node) => {
                const element = node;
     ...
     */
    generateCleanTextContent(markdown) {
