...
export class ContextFormatterService {
     ...
     */
    buildDocumentHierarchy(results) {
     ...
     */
    determineDocumentLevel(url, allResults) {
        try {
     ...
     */
    findBestParent(docNode, allNodes) {
        const candidates = allNodes.filter(node => node.level < docNode.level &&
     ...
     */
    isLikelyParent(parentUrl, childUrl) {
        try {
     ...
     */
    getDocumentPriority(title, url) {
        const lowerTitle = title.toLowerCase();
        const lowerUrl = url.toLowerCase();
        // Highest priority documents
        if (lowerTitle.includes('introduction') || lowerTitle.includes('overview'))
            return 1;
        if (lowerTitle.includes('getting started') || lowerTitle.includes('quickstart'))
            return 2;
        if (lowerTitle.includes('installation') || lowerTitle.includes('setup'))
            return 3;
        if (lowerTitle.includes('tutorial') || lowerTitle.includes('guide'))
            return 4;
        // URL-based priorities
        if (lowerUrl.includes('getting-started') || lowerUrl.includes('quickstart'))
            return 2;
        if (lowerUrl.includes('installation') || lowerUrl.includes('setup'))
            return 3;
        // Default priority
        return 10;
    }
     ...
     */
    createSummaryContent(content, maxLength) {
     ...
     */
    generateTableOfContents(sections, lines, level) {
        for (const section of sections) {
     ...
     */
    generateSectionContent(section, lines, options, level) {
        // Add section header
     ...
     */
    async formatToSummary(results, options = {}) {
     ...
     */
    async saveToFile(content, baseUrl, format = 'full', options) {
        try {
