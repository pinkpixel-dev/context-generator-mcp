...
export class PlatformDetectorService {
     ...
     */
    async detectPlatform(url, htmlContent) {
     ...
     */
    detectFromHeuristics(url, htmlContent) {
        let scores = {
            gitbook: 0,
            docusaurus: 0,
            vuepress: 0,
        ...
        if (url.includes('/guide/'))
            scores.vuepress += 2;
        if (url.includes('.gitbook.'))
            scores.gitbook += 3;
        if (url.includes('mintlify'))
            scores.mintlify += 3;
        ...
        if (htmlContent) {
            ...
            if (classes.includes('docusaurus'))
                scores.docusaurus += 5;
            if (classes.includes('gitbook'))
                scores.gitbook += 5;
            if (classes.includes('vuepress'))
                scores.vuepress += 5;
            if (classes.includes('mintlify'))
                scores.mintlify += 5;
            // Navigation structure hints
            if ($('.book-summary').length > 0)
                scores.gitbook += 3;
            if ($('.navbar__brand').length > 0)
                scores.docusaurus += 3;
        ...
        const topPlatform = Object.entries(scores).reduce((a, b) => scores[a[0]] > scores[b[0]] ? a : b)[0];
